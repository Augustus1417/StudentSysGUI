...
def login():
    global user_id
    user_id = login_page.id_entry.get()
    message, login_status = Login.login(user_id)
    login_page.msg_lbl.config(text=message)
    if login_status:
...
class Login_Page:
    def __init__(self):
        ...
        self.id_entry = Entry(login_container, width=14, font=("Cascadia Code", "15"), relief=FLAT)
        self.id_entry.pack()
        Button(login_container, text="Login", command=login, bg="#4f627e", fg="white", font=("Cascadia Code", "14"), relief=FLAT).pack(pady=(20,5))
        self.msg_lbl = Label(login_container, text="", font=("Cascadia Code", "14"), bg="#d4d4d4", fg="black")
        self.msg_lbl.pack()
